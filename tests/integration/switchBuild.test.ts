...
beforeEach(async () => {
  tempDir = await mkdtemp(join(tmpdir(), "doublemint-switch-"));
});
...
describe.skipIf(!hasGpp)("switch statements", () => {
  it("compiles and runs string switch cases", async () => {
    ...
    expect(run.stdout.trim()).toBe("yes");
  }, 15000);
});
