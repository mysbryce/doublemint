...
beforeEach(async () => {
  tempDir = await mkdtemp(join(tmpdir(), "doublemint-if-"));
});
...
describe.skipIf(!hasGpp)("if statements", () => {
  it("compiles and runs the selected branch", async () => {
    ...
    expect(run.stdout.trim()).toBe("good");
  });
});
