...
beforeEach(async () => {
  tempDir = await mkdtemp(join(tmpdir(), "doublemint-mint-builtins-"));
});
...
describe.skipIf(!hasGpp)("mint built-ins", () => {
  it("compiles and runs math plus io output", async () => {
    ...
    await writeFile(
      ...
      export function main(): void {
        let value: int = Math.roundToInt(Math.sqrt(Math.pow(3.0, 2.0) + Math.pow(4.0, 2.0)));
        println("value=", value);
      }
      `.trimStart(),
      "utf8"
    ...
    expect(run.stdout.trim()).toBe("value=5");
  });
});
